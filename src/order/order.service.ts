...
export class OrderService {
    ...
    async orderCart(userId: string, createOrderCartDto: CreateOrderCartDto): Promise<IOrder> {
        ...
        try {
            ...
            // update user cart to PAID
            await this.cartModel.updateOne({ _id: userCart._id }, { status: CART_STATUS.PAID }, { session });
            ...
            await this.orderModel.updateOne({ _id: order._id }, { orderItems: orderItemIds, subTotal: subTotal, total: total }, { session });
    ...
    async find(): Promise<IOrder[]> {
        return this.orderModel.find().populate('orderItems', null, null, { populate: { path: 'product' } });
    }
